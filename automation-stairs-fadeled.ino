...
Bounce pir2;
Bounce sw;
byte ledPins[] = {13, 12, 11, 10, 9, 8, 7, 6, 5, 4, 3, 2, 44, 45};
//define led strips as FadeLed objects
FadeLed leds[] = {13, 12, 11, 10, 9, 8, 7, 6, 5, 4, 3, 2, 44, 45};
...
int onTime = 30000; // max on duration in ms
...
void initialize() {
  for(byte i = 0; i < elements(ledPins); i++){
    Serial.print("Turining off: ");
...
void upDown() {
  for(byte i = elements(leds); i > 0; i--){
    if(leds[i-1].done()) leds[i-1].setTime(((elements(leds)-i+1)*fadeTime), true);    
    if(!leds[i-1].get()) leds[i-1].on();
  }
}
void downUp() {
  for(byte i = 0; i < elements(leds);i++){
      if(leds[i].done()) leds[i].setTime(((i+1)*fadeTime), true);
      if(!leds[i].get()) leds[i].on();
    }
}
